...
Defines the synchronous REST endpoints exposed by the Spring Boot Kotlin main-service. These endpoints bridge HTTP clients (the demo frontend) to the asynchronous Kafka request/response topics handled by multiagent-service, using correlation IDs to map async replies back to the originating HTTP call.
