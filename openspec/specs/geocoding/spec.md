...
Defines the geocoding capability: turning a free-text address or place name into a list of `{ latitude, longitude, display_name }` candidates suitable for autocomplete, plus the Kafka handler that exposes it across service boundaries.
...
- **AND** 參數 SHALL 包含 `q`（查詢字串）、`format=json`、`limit`（與呼叫端 `limit` 對應，clamp 至 10）
- **AND** request header SHALL 包含自定義 User-Agent 以遵守 Nominatim 使用條款

#### Scenario: API 失敗
- **WHEN** Nominatim API 回傳錯誤或逾時
- **THEN** SHALL 記錄 error log 並回傳空 list
