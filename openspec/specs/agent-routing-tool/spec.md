...
定義 multiagent-service 透過 MCP server 暴露 `plan_route` tool、Gemini chat agent 整合、以及 chat handler 把 agent 結果回送 `chat.response` 的需求規範，讓使用者的自然語言訊息能被自動辨識為路線意圖並回傳結構化路徑。
