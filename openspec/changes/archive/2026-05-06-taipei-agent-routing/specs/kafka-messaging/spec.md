...
- **THEN** message key SHALL 為原始請求的相同 correlation ID，value SHALL 為 JSON，包含：
  ...
  - `reply`（string，必填）— Gemini agent 回覆的自然語言訊息
...
- **THEN** message value SHALL 為 JSON，至少包含：
  - `correlation_id`（string，必填）
  - `routes`（array，必填，可為空陣列）— 每個元素為 `{ path: int[], edges: int[], road_names: string[], estimated_time_min: float, distance_km: float, speed_cameras: object[] }`
...
<!-- 註：原 spec 的「交通指標訊息格式」scenario 與 `traffic.metrics` 條目於本次 MODIFIED 一併移除（YOLO 訊號來源不再使用，graph 動態權重統一由 TDX polling 提供）。詳見 design.md D8 與 proposal 中的 BREAKING bullet。 -->
...
- **AND** 若 `KAFKA_SUBSCRIBE_TOPICS` 未設定、為空字串、或全為空白，SHALL 使用預設清單 `chat.request,route.request`
