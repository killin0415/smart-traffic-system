...
系統 SHALL 定時從 TDX Live Section API 拉取高雄市即時路段車速資料。

#### Scenario: 定時輪詢
- **WHEN** multiagent-service 啟動後
- **THEN** SHALL 每 1-5 分鐘（可設定）向 TDX `GET /v2/Live/Road/Traffic/Section/Kaohsiung` 發送請求
...
Traffic Agent SHALL 根據即時車速計算 congestion factor，並更新 in-memory graph 的 edge weight。

#### Scenario: 有即時資料時計算 congestion factor
- **WHEN** 取得某路段的即時車速 `current_speed` 且 `current_speed > 0`
...
#### Scenario: 即時車速為 0 或負數
- **WHEN** 取得某路段的即時車速 `current_speed ≤ 0`
- **THEN** congestion_factor SHALL 為 `10.0`

#### Scenario: 無即時資料的 edge
- **WHEN** 某 edge 沒有對應的 TDX Live 資料
- **THEN** congestion_factor SHALL 維持 `1.0`（使用 base_weight）

### Requirement: TDX Section ID 與 Edge 的 Mapping
系統 SHALL 透過 `TrafficEdge.tdx_section_id` 欄位將 TDX Live 資料 mapping 到路網 edge。
...
#### Scenario: 無法 mapping
- **WHEN** TDX Live 資料的 SectionID 在 TrafficEdge 中找不到對應紀錄
